...
// проверка LRC для Modbus ASCII
unsigned char CheckLRC(unsigned char* ptr,unsigned short lng)
{
    unsigned char uchLRC=0;
    unsigned short tmp;
    for(tmp=0;tmp<lng-1;tmp++) uchLRC+=ptr[tmp];
    uchLRC=255-uchLRC;
    uchLRC++;
    if(uchLRC==ptr[lng-1]) return 1;
    return 0;
}

// рассчёт LRC для Modbus ASCII
unsigned char getLRC(unsigned char* ptr,unsigned short lng)
{
    unsigned char uchLRC=0;
    unsigned short tmp;
    for(tmp=0;tmp<lng;tmp++) uchLRC+=ptr[tmp];
    uchLRC=255-uchLRC;
    uchLRC++;
    return uchLRC;
}
